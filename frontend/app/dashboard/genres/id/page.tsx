        ...
        <div className="flex-1 overflow-hidden">
            ...
            {data && 
                <div className="bg-gradient-to-r from-slate-900 via-purple-900 to-slate-900 p-8 min-h-screen shadow-md">
                    <h1 className="text-3xl font-semibold mb-4 text-white flex justify-center">{data.getGenreDetails.name}</h1>
                    <img 
                        className="w-full h-64 object-cover rounded-lg mb-4" 
                        src={`${data.getGenreDetails.image_background}`}
                    />
                    <p className="text-gray-700 dark:text-gray-300">{removeHTMLText(data.getGenreDetails.description)}</p>
                    <div className="mt-4">
                      <h1 className="text-gray-600 dark:text-gray-400">{data.getGenreDetails.games_count}</h1>
                    </div>
                </div>
            }
        </div>
